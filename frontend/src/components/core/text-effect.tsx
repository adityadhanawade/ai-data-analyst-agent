...
const getTransition = (
  ...
  if (typeof variant === "object" && variant !== null && "transition" in variant) {
    return (variant as { transition?: Transition }).transition;
  }
  return undefined;
};
...
export function TextEffect({
  ...
  const containerVisibleTransition = getTransition(variants?.container?.visible ?? {}) as
    | { staggerChildren?: number; delayChildren?: number }
    | undefined;

  const customStagger = containerVisibleTransition?.staggerChildren;
  const customDelay = containerVisibleTransition?.delayChildren;
